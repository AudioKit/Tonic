...
public struct Note: Equatable, Hashable {
    ...
    public func shiftDown(_ shift: Interval) -> Note? {
        ...
        let newLetterIndex = (spelling.letter.rawValue - (shift.degree - 1))
    ...
    public var index: Int {
        (octave+1) * 7 * 5 + spelling.letter.rawValue * 5 + (Int(spelling.accidental.rawValue)+2)
    }
}

extension Note: Comparable {
    public static func < (lhs: Note, rhs: Note) -> Bool {
        (lhs.spelling.letter, lhs.spelling.accidental, lhs.octave) < (rhs.spelling.letter, rhs.spelling.accidental, rhs.octave)
    }
}
